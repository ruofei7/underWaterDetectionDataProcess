...
class evaluation(object):
    def __init__(self, cfg):
        #detection path,ground truth path
    ...
    def get_tp_fp(self, gt_path, prediction_path, groundtruths, predictions, label, file_format):
        ...
        for groundtruth in groundtruths:
            ...
            self.all_num_pos += num_pos #=TP+FP 在groundtruth上所有海胆有多少个？比如算海胆的时候
            state_all += det_state
        ...
        for index, pair in enumerate(tp_copy):
            ...
            if tp_copy[index][0] > self.threshold:
                ...
                if tp_copy[index][1] == 1:
                    tp_th += 1
        if(float(tp_th)==0 or float(tp_th_num)==0):
            print('tp_th',float(tp_th),'tp_th_num',float(tp_th_num))
    ...
    def computeAp(self, label):
        ...
        if self.roc:
            ...
            plt.draw()  # 显示绘图
            # plt.pause(5)  # 显示5秒
            plt.savefig("class_{}_roc.jpg".format(label + 2))  # 保存图象
            plt.close()
        ...
        if self.pr:
            ...
            plt.draw()  # 显示绘图
            # plt.pause(5)  # 显示5秒
            plt.savefig("class_{}_pr.jpg".format(label + 2))  # 保存图象
            plt.close()
    ...
    def run(self,result_eval):
        ...
        # print(file_format)
        aps = 0
        for label in range(1, self.cls+1):
            ...
            if label==1:
                ...
                raise ValueError('false class name')
            result_eval[label]['false_detection'].append(round(1-precision,5))
            result_eval[label]['miss_detection'].append(round(1-recall,5))
            result_eval[label]['ap'].append(round(ap,5))
            if self.precision:
                print("class ", class_name, "precision: ", precision)
                print("class ", class_name, "误检率: ", 1-precision)
            if self.recall:
                print("class ", class_name, "recall: ", recall)
                print("class ", class_name, "漏检率: ", 1-recall)
            if self.FPPIW:
